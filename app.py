...
# 1. 페이지 설정
# =============================================================================
st.set_page_config(page_title="통합 자금 관리 대시보드", layout="wide")
...
if choice == "➕ 새 작업 월":
    nm = st.sidebar.text_input("월 (YYYY-MM)", datetime.now().strftime("%Y-%m"))
...
st.sidebar.markdown("---")
if st.sidebar.button("🗑️ 초기화", type="primary"):
    try:
...
uploaded = st.sidebar.file_uploader("파일 업로드", accept_multiple_files=True)
if uploaded:
    for f in uploaded:
...
st.sidebar.markdown("---")
with st.sidebar.expander("⚙️ 컬럼 설정"):
    ...
    st.caption("은행")
    bank_date = st.text_input("날짜", "거래일시", key="bank_date_input")
    bank_desc = st.text_input("내용", "적요")
    bank_out = st.text_input("출금", "출금", key="bank_out_input")
    bank_in = st.text_input("입금", "입금", key="bank_in_input")
...
# [수정된 부분] 파일 상태 표시 로직 개선
st.sidebar.markdown("---")
with st.sidebar.expander("📋 파일 상태", expanded=False):
    for s in status_list:
        if s["ok"]:
            # st.sidebar.success 대신 st.success 사용 -> expander 안에 포함됨
            st.success(f"✅ {s['file']}")
        else:
            st.error(f"❌ {s['file']}")
...
# 6. 탭 구성
# =============================================================================
tab1, tab2, tab3 = st.tabs(["💰 월별 정산", "📊 데이터 통합 확인", "📋 파일 검증"])
...
with tab1:
    ...
    if merged.empty:
        ...
        if '사업장' in merged.columns:
            ...
            selected_company = st.radio("사업장", company_options, horizontal=True)
        ...
        with t_manual:
            ...
            manual_height = b5.slider("높이", 300, 1500, 500, 100, label_visibility="collapsed")
